...
class VPLSubWindow(NodeEditorWidget):
    def __init__(self):
        super().__init__()
    ...
    def handleNewNodeContextMenu(self, event):
        ...
        if action is not None:
            new_vpl_node = get_class_from_opcode(action.data())(self.scene)
            scene_pos = self.scene.getView().mapToScene(event.pos())
            new_vpl_node.setPos(scene_pos.x(), scene_pos.y())
            print('Selected node:', new_vpl_node)
